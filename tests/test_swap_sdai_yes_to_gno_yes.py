...
def main():
    parser = argparse.ArgumentParser(description="Test swapping sDAI YES tokens to GNO YES tokens")
    parser.add_argument("--amount", type=float, default=0.0001, help="Amount to swap (default: 0.0001)")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose output")
    args = parser.parse_args()
    
    print(f"=== Testing sDAI YES to GNO YES Swap (Amount: {args.amount} sDAI YES) ===")
    ...
    # token_type='company' means GNO, is_buy=True means buying GNO tokens, is_yes_token=True means YES tokens
    result = bot.execute_swap(token_type='company', is_buy=True, amount=args.amount, is_yes_token=True)
    
    if result:
        print(f"\n✅ sDAI YES to GNO YES swap test completed successfully")
    else:
        print(f"\n❌ sDAI YES to GNO YES swap test failed")
